...
function generalize_water() {
    ...
    echo "CREATE TABLE $target_table_name AS SELECT ST_Buffer(ST_Simplify(geometry, $tolerance), 0.0) AS geometry FROM $source_table_name" | exec_psql
    echo "UPDATE $target_table_name set geometry = st_multi(st_collectionextract(St_makevalid(geometry),3)) where st_isvalid(geometry) = false" | exec_psql
